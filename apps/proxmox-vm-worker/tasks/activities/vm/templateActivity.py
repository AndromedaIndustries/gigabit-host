...
async def update_proxmox_templates() -> bool:
    ...
    templateDatabase = "ProxmoxTemplates"

    if os.environ.get("DEBUG") == "true":
        print("Updating Proxmox templates...")

    proxmox: ProxmoxAPI = get_proxmox_client()
    if proxmox is None:
        raise ValueError("Proxmox client is Failed to create")

    dbClient: Client = get_supabase_client()
    if dbClient is None:
        raise ValueError("Supabase client is Failed to create")
        ...
        for vm in proxmox.nodes(node["node"]).qemu.get():
            if vm["name"].startswith("template-"):
                ...
                try:
                    ...
                    template = (
                        dbClient.table(templateDatabase)
                        .select("*")
                    ...
                    # If the template is not in the database, insert it
                    pass
                ...
                try:
                    ...
                    print(f"Failed to insert template into database: {e}")
                    return False
